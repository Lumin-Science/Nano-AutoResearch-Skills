...
2. Convert it:
   ...
   PDF conversion keeps a faithful Poppler-rendered page underneath positioned, independently clickable modules. Poppler bounding boxes are grouped into headings, prose, equations, and table regions; embedded raster images become figure regions. The positioned word spans remain selectable and the selected block identity/text travels with the comment. If positioned extraction fails, the converter warns and falls back to the coarse page-wide text layer. Scanned pages need OCR, and vector-only plots may have only caption/text modules over the page image.
